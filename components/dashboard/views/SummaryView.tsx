...
export default function SummaryView({ data }: SummaryViewProps) {
  ...
  return (
    <table className="w-full text-left border-collapse min-w-[1700px] text-sm">
      <thead className="bg-slate-50/50 sticky top-0 z-10 shadow-[0_1px_0_0_#f1f5f9]">
        <tr>
          ...
          <tr key={`${row.name}-summary-${i}`} className="hover:bg-slate-50/80 transition-colors animate-in fade-in duration-300">
            ...
            <td className="px-6 py-4">
               <div className="flex flex-col">
                  ...
                  <div className="flex items-center gap-1 text-[11px] text-slate-400 mt-1 max-w-[180px]">
            ...
            <td className="px-6 py-4 text-right text-slate-400 tabular-nums">{formatNum(row.product1Amount)}</td>
            <td className="px-6 py-4 text-right text-slate-400 tabular-nums">{formatNum(row.product2Amount)}</td>
            <td className="px-6 py-4 text-right text-slate-400 tabular-nums">{formatNum(row.product3Amount)}</td>
            ...
            <td className="px-6 py-4 text-right">
                <div className="flex flex-col items-end gap-0.5">
                  <span className={`text-[1.1rem] font-black tabular-nums tracking-tight ${row.totalIncentiveLC > 0 ? 'text-emerald-600' : 'text-slate-300'}`}>
                    {row.totalIncentiveLC > 0 ? formatNum(row.totalIncentiveLC) : "0"}
                  </span>
                  <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">LC</span>
                </div>
            </td>
          </tr>
        ))}
